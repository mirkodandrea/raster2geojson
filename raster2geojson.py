...
def smooth_polygon(poly: Polygon, smooth_sigma: float) -> Polygon:
    ...
    Returns:
        Polygon: The smoothed polygon as a MultiPolygon object.
    """
...
def shape_from_raster(
    ...
    # Remove MultiLineStrings and LineStrings that can be generated by the processing
...
@click.option('--format', default='GeoJSON', help='Output format', type=click.Choice(['GeoJSON', 'shapefile', 'ESRI Shapefile', 'GPKG']))
def main(file_in, file_out, holes, mask, values, smooth, format):
    ...
    elif format.lower() in ['shapefile', 'esri shapefile']:
